...
const Profile = () => {
  ...
  async function updateEssay(){
    await axios.post('/user/update', { essay }).then(data => {
      setUser({...user, essay })
      setEssay('');
      onClick();
    })
  ...
  return (
    <div className="profile">
      ...
      <div className="essay">{ user.essay } <button onClick={onClick}><i className="fas fa-undo"></i></button></div>
      <div className="input-essay hidden">
        <textarea name="essay" type="text" placeholder={user.essay} onChange={onChange} value={essay}/>
        <div className="btns">
