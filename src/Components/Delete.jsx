...
const Delete = () => {
  ...
  useEffect(() => {
    if (answer && count === answer) {
      const choice = window.confirm(
        "Aere you sure you want to delete this account?"
      );
  ...
  async function deleteAccount() {
    await axios
      .get("https://dating-app-clone.herokuapp.com/user/delete", { withCredentials: true })
      .then((data) => {
        if (data.status === 200) {
  ...
  return (
    <div className="delete">
      <h1>
        Click Image {firstNum} + {secondNum} times
      </h1>
